...
- Internal control-plane endpoints are demo-internal only. They are not part of the public API surface.
...
- Do not imply BTCPay billing or account balance top-ups are implemented unless that work is actually added later.
...
- `Is this domain really being verified live?`
  The onboarding UX is real product shape. The live traffic portion intentionally uses a pre-verified demo domain so the request-path proof is deterministic during review.

- `Are those analytics real time?`
  They are derived from the request events generated in the demo. The proof panel is the immediate source of truth, and analytics are the confirmation layer.
